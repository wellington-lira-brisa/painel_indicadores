...
export default function PaginaRanking({ tecnologia = TECNOLOGIAS.ftth }) {
  ...
  const totais = {
    ...
    semDado: cidadesFiltradas.filter((c) => c.status === 'sem-dado').length,
  };
  ...
  return (
    <div className={`space-y-6 ${tecnologia.classeTema}`}>
      <section aria-label="Resumo geral" className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <CardKpi titulo="Atingimento médio" valor={formatarPercentual(mediaGeral)} destaque />
        <CardDistribuicaoStatus totais={totais} />
      </section>
