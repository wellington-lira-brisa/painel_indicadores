...
test('normalizarMetaPorCanal: aplica multiplicador e soma os indicadores de "instalacao" por cidade+canal+mês', () => {
  ...
  assert.equal(loja.indicadorId, 'instalacao');
  const pap = registros.find((r) => r.canal === 'PAP');
  assert.equal(pap.meta, 5); // avulso, multiplicador 1
});

test('normalizarMetaPorCanal: categorias diferentes (orcamento/efetivado/instalacao/ativacao) nunca se misturam, mesma cidade+canal+mês', () => {
  const { indice } = indexarMultiplicadoresDicionarioMetas([
    { data: '2026-01-01', indicador: 'Vendas criadas avulso - FTTH', FTTH: '1', FWA: '0', '5G': '0' },
    { data: '2026-01-01', indicador: 'Vendas efetivadas avulso - FTTH', FTTH: '1', FWA: '0', '5G': '0' },
    { data: '2026-01-01', indicador: 'Vendas instaladas avulso - FTHH', FTTH: '1', FWA: '0', '5G': '0' },
  ...
  const { registros, avisos } = normalizarMetaPorCanal(linhasFato, indice);
  assert.deepEqual(avisos, []);
  assert.equal(registros.length, 4); // 1 registro por categoria, nunca somado entre categorias
  assert.equal(registros.find((r) => r.indicadorId === 'orcamento').meta, 10);
  assert.equal(registros.find((r) => r.indicadorId === 'efetivado').meta, 8);
  assert.equal(registros.find((r) => r.indicadorId === 'instalacao').meta, 6);
  assert.equal(registros.find((r) => r.indicadorId === 'ativacao').meta, 4);
});

test('normalizarMetaPorCanal ignora silenciosamente indicador fora de qualquer categoria conhecida (ex.: Churn)', () => {
  const { indice } = indexarMultiplicadoresDicionarioMetas([
    { data: '2026-01-01', indicador: 'Churn Safra - Banda Larga', FTTH: '0', FWA: '0', '5G': '0' },
  ]);
  const linhasFato = [
    { data: '2026-01-01', canal: 'LOJA', indicador: 'Churn Safra - Banda Larga', meta: '10', cidade: 'ACOPIARA/CE' },
  ];
  const { registros, avisos } = normalizarMetaPorCanal(linhasFato, indice);
  assert.equal(registros.length, 0);
  assert.deepEqual(avisos, []); // não é erro, é indicador fora de escopo — não gera aviso
});

test('normalizarMetaPorCanal descarta (com aviso) linha sem regra no dicionário pro mês', () => {
...
test('normalizarMetaPorCanal ignora linha sem cidade mapeável (nunca inventa cidade)', () => {
  const { indice } = indexarMultiplicadoresDicionarioMetas([
    { data: '2026-01-01', indicador: 'Vendas instalada Combo - FTTH', FTTH: '1', FWA: '0', '5G': '1' },
  ]);
  const linhasFato = [
...
test('paraCsvMetaPorCanal + parsearCsv fazem roundtrip sem perda, indicador_id vem do registro', () => {
  const { indice } = indexarMultiplicadoresDicionarioMetas([
    { data: '2026-01-01', indicador: 'Vendas Ativadas - 5G', FTTH: '0', FWA: '0', '5G': '1' },
  ]);
