...
function obterTodosOsFeriadosParaAno(ano, uf, municipio, deveMarcarEmendas) {
  ...
  if (municipio == undefined || municipio == null || municipio == "") {
    ...
    // Correção deliberada em relação ao original vendorizado: se o
    // município não está nos 238 cadastrados com feriado próprio (a
    // maioria dos municípios do Brasil não está), o original tentava ler
    // `.feriados` de `undefined` e quebrava. A nova página de Calendário
    // de Feriados depende de conseguir consultar QUALQUER cidade e cair
    // graciosamente em "sem feriado municipal" quando ela não está na
    // base — daí o guard abaixo. Nenhuma outra regra de cálculo mudou.
    municipais = municipio == undefined ? [] : calcularFeriadosDoAnoParaLista(municipio.feriados, "MUNICIPAL", ano);
  }
...
// `estados` e `feriadosNacionais` passaram a ser exportados também: a nova
// página de Calendário de Feriados precisa enumerar todos os municípios
// cadastrados (pra busca por cidade) e listar feriados nacionais sem exigir
// UF/cidade selecionada — nenhuma lógica de cálculo muda, só passam a ser
// visíveis fora deste arquivo.
export { obterTodosOsFeriadosParaAno, estados, feriadosNacionais };